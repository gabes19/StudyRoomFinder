...
from app.models import Base, Library, Room

# FOR AWS LAMBDA, environment variable will be configured via their console
...
class LibraryScraper:
    def __init__(self, library_name):
        self.name = library_name  # String name of the Library
    ...
    def collect_all_times(self):
        ...
        location_dropdown = Select(
            driver.find_element(By.ID, "s-lc-location")
        ).select_by_visible_text(self.name)
        ...
        for capacity in room_capacities:
            # Select capacity
            capacity_dropdown = Select(
                driver.find_element(By.ID, "s-lc-type")
            ).select_by_visible_text(capacity)
            # Create String list to prevent Stale Element Reference
            space_dropdown_list = Select(
                driver.find_element(By.ID, "s-lc-space")
            ).options
            room_names = []
            ...
            rooms_at_this_capacity = self.get_room_data(
                driver.current_url, capacity, room_names
            )
